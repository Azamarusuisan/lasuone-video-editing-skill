...
- GitHub保存と検証の最新記録は`GIT_CHECKPOINT.md`。保存対象はコード・文書・解析JSON等で、研究動画・音声・画像とローカル実行環境は含めない。取得済み188本の映像解析・概要capture・OCR、音量計測、未校正ASR候補が揃い、機械バッチは完走。COVERAGE/草案は11:31:34Zの凍結集計へ同期した。
...
- 取得済み188本の機械処理は完走済み。旧PIDは終了した記録であり、再開時に存在や所有を仮定しない。
...
最新保存集計: `evidence/COVERAGE.json` compiled_at=`2026-09-14T11:31:34.991Z`（JST 9/14 20:31:34）。
...
| 研究コピー取得 | 188 / 189本 |
| 未校正ASR候補あり | 188本 |
| 全尺フレーム差分候補の機械解析 | 188本 |
| 何らかの部分目視確認 | 56本 |
| 全尺に分散した静止サンプルを確認 | 28本（56本の内数） |
| 初期字幕DB | 30本の部分画像から114字幕フレーム |
| 局所の連続全フレーム確認 | 21区間 / 551フレーム |
| 全編の連続映像・音声確認完了 | **0本** |

114は独立字幕イベント数ではなく、同じ字幕の保持フレームを含む。56本を全編視聴したとは絶対に報告しない。全体進捗を56/189や188/189だけで学習完成率に換算しない。
...
**2026-09-14 Codex追記**: R9/rUZ/Z53/N2の概要グリッドは全て確認済み。COVERAGEへ集計される全尺分散reviewは28本。次は未確認の通常動画とShorts、元回/総集編の対応、重要箇所の局所連続フレームを進める。以下のR9節は旧記録としてのみ参照する。
...
- `rUZ__0uKiMA` 3:19:39 — overview全134シート・799画像を公称0〜11970秒で確認完了。最後の約8.57秒、静止点間、音声は未確認。選定26点も別途確認済み。
- `Z53j2cRj1C8` 4:24:43 — overview全177シート・1059画像を公称0〜15870秒で確認完了。最後の約12.81秒、静止点間、音声は未確認。選定32点も別途確認済み。
...
2026-09-14 20:34 JST頃に読み取り確認。研究コピー188本について、映像の全尺差分候補、source PTS概要、OCR、音量計測、未校正ASR候補が揃った。映像・音声バッチの該当プロセスは終了済み。

- 古い親PID57549/57829、19964/19965とPTY番号31807/41953/94233は完了済みの記録。存在を前提にせず、再利用されたPIDへ操作しない。
- 再実行前に現ファイルとCOVERAGEを照合し、同一動画を重複投入しない。
- 機械処理完了は目視・聴取・ASR校正・全編確認の完了ではない。
